...
class GeneralizedDataset:
    ...
    def check_dataset(self, checked_id_file):
        ...
        print("{} samples are OK; {:.1f} seconds".format(
            len(self),
            time.time() - since))
        ...
        for i in seq:
            ...
            try:
                ...
                assert len(boxes) == len(
                    labels), "{}: len(boxes) != len(labels)".format(i)
                assert len(boxes) == len(
                    masks), "{}: len(boxes) != len(masks)".format(i)
...
class COCODataset(GeneralizedDataset):
    def __init__(self, data_dir, split, train=False):
        ...
        self.ids = [str(k) for k in self.coco.imgs] + [
            str(k) + "_aug" for k in self.coco.imgs
        ] + [str(k) + "_2" for k in self.coco.imgs]
        # self.ids = [str(k) for k in self.coco.imgs]
        # print(len(self.ids))

        self._classes = {k: v["name"] for k, v in self.coco.cats.items()}
        self.classes = tuple(self.coco.cats[k]["name"]
                             for k in sorted(self.coco.cats))
        # resutls' labels convert to annotation labels
        # self.ann_labels = {self.classes.index(v): k for k, v in self._classes.items()}

        checked_id_file = os.path.join(data_dir,
                                       "checked_{}.txt".format(split))
        if train:
    ...
    def get_image(self, img_id):
        ...
        image = Image.open(
            os.path.join(self.data_dir, "{}".format(self.split),
                         img_info["file_name"]))
    ...
    def get_target(self, img_id):
        ...
        target = dict(image_id=torch.tensor([img_id]),
                      boxes=boxes,
                      labels=labels,
                      masks=masks,
                      iscrowd=iscrowd,
